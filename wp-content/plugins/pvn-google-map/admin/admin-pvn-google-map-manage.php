        ...
        jQuery(function () {
            ...
            new Clipboard('.copy-shortcode', {
                text: function (trigger) {
                    return '[pvn-google-map id="' + trigger.getAttribute('data-id-copy') + '"]';
                }
            });
...
if(isset($_GET[ 'create_map' ])){
    ...
    if(!in_array($_GET[ 'map_id' ], $arr_map_id)){
        $wpdb->insert($wpdb->prefix . 'mapmarker_option',
            array(
                'map_id'                 => $_GET[ 'map_id' ],
                'map_name'               => __('Untitled map', 'pvn-google-map') . ' ' . $_GET[ 'map_id' ],
...
if(isset($_GET[ 'edit_map' ]) || isset($_GET[ 'create_map' ])){
    ...
    <div class="wrap">
        <div class="head-map-detail">
            <div class="row">
                <div class="col-md-4">
                    <a href="?page=pvn-google-map" class="button"><i class="fa fa-chevron-circle-left" aria-hidden="true"></i> <?php _e("Back", "pvn-google-map") ?>
                    </a>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    <div class="pull-right">
                        <div class="button copy-shortcode" data-id-copy="<?php echo $_GET[ 'map_id' ] ?>" title="<?php _e("Click to copy the shotcode", "pvn-google-map") ?>">
                            <?php _e("Copy", "pvn-google-map") ?> : <b>[pvn-google-map
    ...
    include_once dirname(__DIR__) . '/admin/admin-pvn-google-map-option.php';

    // Charge les marqueurs de la map
    include_once dirname(__DIR__) . '/admin/admin-pvn-google-map.php';
    ...
    if(isset($_POST[ 'securite_nonce_delete_map' ])){
        ...
        if(wp_verify_nonce($_POST[ 'securite_nonce_delete_map' ], 'securite-nonce')){
            ...
            echo '<div class="wrap">';
            mapmarker_message_alert(__('Your map been deleted.', 'pvn-google-map'), 'updated');
            echo '</div>';
        }
    }
    ...
    ?>
    <div class="wrap">
        <h2><?php _e("Manage Map", "pvn-google-map") ?></h2>
        <p></p>
        <table class="widefat" id="table_map">
            <thead>
            <tr>
                <th><strong><?php _e("Map Name", "pvn-google-map") ?></strong></th>
                <th><strong><?php _e("Shortcode", "pvn-google-map") ?></strong></th>
                <th></th>
            </tr>
            ...
            <?php foreach($map_id as $value): ?>
                <tr>
                    ...
                    <td>
                        <div>
                            <pre>[pvn-google-map id="<?php echo $value[ 'map_id' ] ?>"]</pre>
                        </div>
                        <a href="#" class="copy-shortcode"
                           data-id-copy="<?php echo $value[ 'map_id' ] ?>"><?php _e('Copy', 'pvn-google-map') ?></a>
                    </td>
                    <td>
                        <a href="?page=pvn-google-map&edit_map&map_id=<?php echo $value[ 'map_id' ] ?>" class="button-primary"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> <?php _e("Edit", "pvn-google-map") ?>
                        </a>
            ...
            <tr>
                <td>
                    <a href="?page=pvn-google-map&create_map&map_id=<?php mapMarkerGetNewMapId(); ?>" class="button-primary button-success"><i class="fa fa-map-o" aria-hidden="true"></i> <?php _e("Create new map", "pvn-google-map") ?>
                    </a></td>
                <td></td>
                <td></td>
            </tr>
