...
if(isset($_POST[ 'securite_nonce_upload_csv' ])){
    if(wp_verify_nonce($_POST[ 'securite_nonce_upload_csv' ], 'securite_nonce_upload_csv')){
        ...
        try{
            ...
            //Display success msg
            mapmarker_message_alert(__('Your csv file has been successfully downloaded.', 'pvn-google-map'), 'updated');

        } catch(Exception $e){
            //Error validation
            if($e->getMessage() == 'ERROR_VALID'){
                //Display error msg
                mapmarker_message_alert(__('Please choose a valid file.', 'pvn-google-map'), 'error');
            } elseif($e->getMessage() == 'ERROR_SQL'){
                mapmarker_message_alert(__('Error SQL.', 'pvn-google-map'), 'error');
            } else{
                //Display error msg
                mapmarker_message_alert(__('Error csv file upload.', 'pvn-google-map'), 'error');
            }
        }
...
if(isset($_POST[ 'securite_nonce_manage_marker' ]) OR isset($_POST[ 'securite_nonce_delete_marker' ])){
    ...
    if(wp_verify_nonce($_POST[ 'securite_nonce_manage_marker' ], 'securite-nonce') OR wp_verify_nonce($_POST[ 'securite_nonce_delete_marker' ], 'securite-nonce')){
        ...
        if(isset($_POST[ 'submit_delete' ])){
            ...
            mapmarker_message_alert(__('Your marker been deleted.', 'pvn-google-map'), 'updated');
        }
        ...
        if(isset($_POST[ 'valid_edition' ])){
            ...
            //Affiche le message d'alert
            mapmarker_message_alert(__('Your marker been edited.', 'pvn-google-map'), 'updated');
    ...
    <div class="panel-heading" role="tab" id="headingTwo">
        <h4 class="panel-title">
            <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseTwo" aria-expanded="true" aria-controls="collapseTwo" class="action-accordeon">
                <i class="fa fa-map-marker" aria-hidden="true"></i> <?php _e('Management marker', 'pvn-google-map') ?>
        ...
        <div class="panel-body">
            <form method="POST" action="" enctype="multipart/form-data" id="form-upload-csv">
                ...
                <button id="upload_csv_submit" type="submit" class="button-primary button-orange">
                    <i class="fa fa-plus" aria-hidden="true"></i> <?php _e('Import .csv', 'pvn-google-map') ?>
                </button>
            </form>
            ...
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <table class="widefat" id="table_markers">
                    <thead>
                    <tr>
                        <th style="text-align: center"><strong><?php _e('Marker', 'pvn-google-map') ?></strong></th>

                        <?php
                        if(mapmarker_get_checked_field($options[ 0 ][ 'fiels_to_display' ], 'image') == TRUE){
                            ?>
                            <th style="text-align: center"><strong><?php _e('Image', 'pvn-google-map') ?></strong>
                            </th>
                            <?php
                        }
                        ?>
                        ...
                        if(mapmarker_get_checked_field($options[ 0 ][ 'fiels_to_display' ], 'titre') == TRUE){
                            ?>
                            <th><strong><?php _e('Title', 'pvn-google-map') ?></strong></th>
                            <?php
                        }
                        ?>
                        <?php
                        if(mapmarker_get_checked_field($options[ 0 ][ 'fiels_to_display' ], 'description') == TRUE){
                            ?>
                            <th><strong><?php _e('Description', 'pvn-google-map') ?></strong></th>
                            <?php
                        }
                        ?>
                        <?php
                        if(mapmarker_get_checked_field($options[ 0 ][ 'fiels_to_display' ], 'adresse') == TRUE){
                            ?>
                            <th><strong><?php _e('Address', 'pvn-google-map') ?></strong></th>
                            <?php
                        }
                        ?>
                        <?php
                        if(mapmarker_get_checked_field($options[ 0 ][ 'fiels_to_display' ], 'telephone') == TRUE){
                            ?>
                            <th><strong><?php _e('Phone', 'pvn-google-map') ?></strong></th>
                            <?php
                        }
                        ?>
                        <?php
                        if(mapmarker_get_checked_field($options[ 0 ][ 'fiels_to_display' ], 'weblink') == TRUE){
                            ?>
                            <th><strong><?php _e('Web link', 'pvn-google-map') ?></strong></th>
                            <?php
                        }
                        ?>
                        <th><strong><?php _e('Latitude', 'pvn-google-map') ?>*</strong></th>
                        <th><strong><?php _e('Longitude', 'pvn-google-map') ?>*</strong></th>
                        <th><strong><?php _e('Action', 'pvn-google-map') ?></strong></th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php
                        ...
                        <tr>
                            ...
                            <td class="action">
                                ...
                                <button type='submit' class='button-primary' name="edit_marker" id="edit_marker">
                                    <i class="fa fa-pencil-square-o" aria-hidden="true"></i> <?php _e('Edit', 'pvn-google-map'); ?>
                                </button>
                                <button type='submit' class='button-secondary' name="delete_marker" id="delete_marker">
                                    <i class="fa fa-times" aria-hidden="true"></i> <?php _e('Delete', 'pvn-google-map'); ?>
                                </button>
                            </td>
                        </tr>
                        <?php
                    ...
                    <tr>
                        ...
                        if(mapmarker_get_checked_field($options[ 0 ][ 'fiels_to_display' ], 'titre') == TRUE){
                            ?>
                            <td>
                                <input maxlength="50" type="text" name="add_titre" placeholder="<?php _e('Title', 'pvn-google-map') ?>">
                            </td>
                            <?php
                        }
                        ?>
                        ...
                        if(mapmarker_get_checked_field($options[ 0 ][ 'fiels_to_display' ], 'adresse') == TRUE){
                            ?>
                            <td>
                                <textarea maxlength="255" name="add_adresse" placeholder="<?php _e('Address', 'pvn-google-map') ?>"></textarea>
                            </td>
                            <?php
                        }
                        ?>
                        ...
                        if(mapmarker_get_checked_field($options[ 0 ][ 'fiels_to_display' ], 'telephone') == TRUE){
                            ?>
                            <td>
                                <input maxlength="12" type="text" name="add_telephone" placeholder="<?php _e('Phone', 'pvn-google-map') ?>">
                            </td>
                            <?php
                        }
                        ?>
                        ...
                        if(mapmarker_get_checked_field($options[ 0 ][ 'fiels_to_display' ], 'weblink') == TRUE){
                            ?>
                            <td>
                                <input maxlength="255" type="text" name="add_weblink" placeholder="<?php _e('Web link', 'pvn-google-map') ?>">
                            </td>
                            <?php
                        }
                        ?>
                        <td>
                            <input maxlength="10" type="text" name="add_latitude" class="requiered" placeholder="<?php _e('Latitude', 'pvn-google-map') ?>">
                        </td>
                        <td>
                            <input maxlength="11" type="text" name="add_longitude" class="requiered" placeholder="<?php _e('Longitude', 'pvn-google-map') ?>">
                        </td>
                        <td>
                            <button type='submit' class='button-primary button-success' name="create_marker" id="create_marker">
                                <i class="fa fa-plus" aria-hidden="true"></i> <?php _e('Add', 'pvn-google-map'); ?>
                            </button>
                        </td>
                    </tr>
                    </tfoot>
