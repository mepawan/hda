...
function mapmarker_plugin_activation(){
    ...
    if(empty($check_option)){
        ...
        $wpdb->insert($table_option,
            array(
                'map_id'                 => '1',
                'map_name'               => __('Example map', 'pvn-google-map'),
                'height_map'             => '500',
    ...
    if(empty($check_option)){
        ...
        $wpdb->insert($table_marker, array(
            'marker_id'       => '1',
            'titre'           => __("Eiffel Tower", "pvn-google-map"),
            'description'     => __("Constructed in 1889 as the entrance to the 1889 World's Fair, it was initially criticized by some of France's leading artists and intellectuals for its design, but it has become a global cultural icon of France...", "pvn-google-map"),
...
function mapmarker_plugin_loaded(){
    ...
    load_plugin_textdomain('pvn-google-map', FALSE, plugin_basename(dirname(dirname(__FILE__))) . '/language');
}
...
function mapmarker_admin_menu(){
    ...
    //add_menu_page('PVN Google Map', 'PVN Google Map', 'manage_options', 'pvn-google-map-menu', '', 'dashicons-location-alt');
    add_menu_page('PVN Google Map', __('PVN Google Map'), 'manage_options', 'pvn-google-map', 'mapmarker_manage', 'dashicons-location-alt');
    // Add. le submenu
    add_submenu_page('pvn-google-map', __('Settings PVN Google Map', 'pvn-google-map'), __('Settings', 'pvn-google-map'), 'manage_options', 'pvn-google-map-settings', 'mapmarker_settings');
   // add_submenu_page('pvn-google-map', __('Help PVN Google Map', 'pvn-google-map'), __('Help', 'pvn-google-map'), 'manage_options', 'pvn-google-map-help', 'mapmarker_help');
}
...
function mapmarker_admin_script(){
    ...
    $translation_array = array(
        'msg_error_empty'     => __('Please choose a file.', 'pvn-google-map'),
        'mssg_error_required' => __('Required fields.', 'pvn-google-map')
    );
    wp_localize_script('mmm-admin-js', '_e', $translation_array);

    //Load sur toute les page du plugin
    if($_GET[ 'page' ] == 'pvn-google-map' OR $_GET[ 'page' ] == 'pvn-google-map-settings' OR $_GET[ 'page' ] == 'pvn-google-map-manage' OR $_GET[ 'page' ] == 'pvn-google-map-help'){
        // Load le script & css
        //Load boostrap
    ...
    // Si les page est "pvn-google-map"
    if($_GET[ 'page' ] == 'pvn-google-map'){
    ...
    if($_GET[ 'page' ] == 'pvn-google-map' AND isset($_GET[ 'edit_map' ]) OR $_GET[ 'page' ] == 'pvn-google-map' AND isset($_GET[ 'create_map' ])){
        //Load boostrap
        wp_enqueue_script('bootstrap-min-js');
    }

    //Si la variable $page contient le text domaine (pvn-google-map) et que la page est contient la variable create_map ou edit_map
...
function mapMarkerCreateMarker(){
    ...
    mapmarker_message_alert(__('Your marker been created.', 'pvn-google-map'), 'updated');
}
            ...
            <div class="wrap-modal-child">
                <div class="modal-confirm">
                    <form action="" method="post">
                        ...
                        <h4><?php _e('Do you confirm the suppression of the map ?', 'pvn-google-map'); ?></h4>
                        <button id="cancel-supp"
                                class="button-secondary"><?php _e('Cancel', 'pvn-google-map'); ?></button>
                        <button type="submit" class="button-primary" name="submit_delete"
                                id="submit_delete"><?php _e('Confirm', 'pvn-google-map'); ?></button>
                    </form>
                </div>
            </div>
        </div>
            ...
            <div class="wrap-modal-child">
                <div class="modal-confirm">
                    <form action="" method="post">
                        ...
                        <h4><?php _e('Do you confirm the suppression of the marker ?', 'pvn-google-map'); ?></h4>
                        <button id="cancel-supp"
                                class="button-secondary"><?php _e('Cancel', 'pvn-google-map'); ?></button>
                        <button type="submit" class="button-primary" name="submit_delete"
                                id="submit_delete"><?php _e('Confirm', 'pvn-google-map'); ?></button>
...
function mapMarker_add_action_links($links){
    ...
    $donate_link = array('<a style="font-weight:bold;color:#4CAF50;" href="' . $mapmarker_info[ 'donate_link' ] . '" target="_blank">' . __('Donate', 'pvn-google-map') . '</a>',);
...
function mapmarker_modal_media_selector(){
    ...
    echo "title: " . wp_json_encode(__('Select a image to upload', 'pvn-google-map')) . ",";
    echo "button: {text: " . wp_json_encode(__('Use this image', 'pvn-google-map')) . ",},";
    ...
    if($apiKey[ 0 ][ 'apikey' ] == '' || $apiKey[ 0 ][ 'apikey' ] == NULL || $apiKey[ 0 ][ 'apikey' ] == $mapmarker_info[ 'default_api_key' ]){
        ...
        <div class="wrap">
            <div class="notice notice-error is-dismissible"><p>
                    <strong>
                        <?php
                        _e('Now, before you start using PVN Google Map, please note that it is necessary to register your API key', 'pvn-google-map');
                        if($_GET[ 'page' ] != 'pvn-google-map-settings'){
                            echo ' <a href="?page=pvn-google-map-settings">' . __('on the setting page', 'pvn-google-map') . '</a> ';
                        } else{
                            echo ' ';
                        }
                        _e('to work properly.', 'pvn-google-map');
                        ?>
                    </strong>
                </p>
